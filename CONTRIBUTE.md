...
- [Before you begin](#before-you-begin) 
    - [Get to know better Mule](#getting-to-know-better-mule) 
...
2. Peruse the [Mule forum](http://forum.mulesoft.org/mulesoft) chatter to see if anyone else has begun to resolve the problem or initiate the improvement.
...
It is therefor important to understand those two extension mechanisms. The first and more intuitive mechanism for extensions is the [Anypoint Connector DevKit](http://www.mulesoft.org/documentation/display/current/Anypoint+Connector+DevKit), an annotation based framework for easy construction of extesions. The second choice is to exten mule through the [Extensions](http://www.mulesoft.org/documentation/display/current/Extending) mechanism.
...
Working directly on the master version of Mule source code would likely result in merge conflicts with the original master. Instead, as a best practice for contributing to source code, work on your project in a topic branch.
...
1. If you are using an IDE, make sure you read the forthcoming section: Working with IDE.
...
In order to set up our [development environment](#setting-up-the-development-environment) we got to [install some prerequisites](#installing-prerequisites). Once we had them ready, we  downloaded the [source code](#getting-the-source-code) and created our very own [topic branch](#creating-your-topic-branch) were we'll develop our improvement.
